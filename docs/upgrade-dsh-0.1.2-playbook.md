...
- `npm view @deepseek-ai/dsh dist-tags` 的 `next` 到达 `0.1.2-alpha.1` 或更高；
...
- 前端插件机制原样：`__ModuleLoader__`/`mode='live'`/`__DSH_BOOT__`/`dsh.client` 扫描/`/plugins/<id>/client.js`。
...
8. **版本 bump**：全部 `@deepseek-ai/*` 依赖升到新发布线；`npm install && npm run typecheck && npm test && npm run build` 全绿后才装进 profile。
...
- [ ] `curl -s <host>:<port>/ | grep -o '__DSH_BOOT__[^<]*'` 含 `dsh-web-startup-auth`；`/plugins/dsh-web-startup-auth/client.js` 返回 200。
