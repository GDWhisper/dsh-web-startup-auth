...
- **可选的人机验证（滑块拼图）**：**应网友要求加上了没什么用的防人机验证，有兴趣可以开启试试 XD** —— 在**设置面板 → 认证 → 人机验证**里滑动开启，登录时会让你把一块拼图拖进缺口。每张图里画了**两个缺口**：拼图块和真缺口是**同一种形状**（凸块方向每张随机：右/左/上/下，也可能没有凸块，共 5 种），旁边的假缺口形状永远不同，拼图块只塞得进形状相同的那个（纯装饰，脚本照样秒过）。
  说它没什么用是有实测依据的：**拼图的答案必须画在图上**（人要看着它拖），所以脚本几十行就能读出来——本插件按常规做法实现的三种变体，用约 30 行脚本攻击的命中率分别是 100%（缺口涂暗）、100%（只描边）、97%（撒噪点，但那一版**连人也解不出来**）。之前试过的 4 位数字图片题同样被模板匹配打到 96.7%。**当个乐子，别当安全边界。** 详见 [docs/agent/human-verification.md](docs/agent/human-verification.md)。
- **登录限流硬化**：每 IP 10 分钟内 5 次失败锁 30 秒，**外加全局指数退避**——所有来源累计失败超过 20 次后按台阶翻倍（5 秒 → 10 秒 → …封顶 5 分钟），用来挡住"一千个 IP 各试几次"的分布式撞库；429 带 `Retry-After`。本机（真回环且未开启「本机登录校验」）豁免全局退避，避免攻击者借此把管理员锁在自己机器外面。
...
4. 退出登录 / 修改用户名 / 修改密码 / 调整会话有效期 / 开关拼图验证：打开主界面**设置面板 → 认证**标签页（同时也是一个独立入口，`/api/auth/logout` 清除会话 cookie）。
...
- 兜底方案：删除凭据文件（`$DSH_HOME/web-auth.json`，默认 `~/.dsh/web-auth.json`）并重启，即可重新注册。这只影响登录状态——已登录的浏览器需要再登录一次，**不会删除任何会话记录或历史数据**（它们存在 `~/.dsh/sessions/` 等位置，与本文件无关）。
...
- 会话有效期默认 14 天，可在设置面板「认证」标签页调节（3/7/14/30/60/90/180 天档位，持久化于凭据文件）；调整只影响新签发的会话，已登录的会话保持签发时的有效期。
