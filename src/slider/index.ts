...
import { createChallengeStore } from '../challenge-store.ts'
import {
  CANVAS_HEIGHT,
  CANVAS_WIDTH,
  START_X,
...
import { renderPuzzle, type SliderTheme } from './render.ts'
...
export type SliderVerdict = { ok: true } | { ok: false; reason: SliderFailure }

/** Live puzzles, keyed by the opaque handle. */
const store = createChallengeStore<{ answer: Slot }>()
 ...
 */
export function issueSlider(clientKey: string | undefined, theme: SliderTheme = 'light'): IssuedSlider {
 ...
 */
export function verifySlider(clientKey: string | undefined, id: string, answer: string): SliderVerdict {
  ...
  if (record.clientKey !== clientKey) return { ok: false, reason: 'ip-mismatch' }
  const submitted = parseDrop(answer)
  if (submitted === undefined) return { ok: false, reason: 'malformed' }
  if (!isWithinTolerance(record.payload.answer, submitted)) return { ok: false, reason: 'misaligned' }
  return { ok: true }
}
 ...
 */
export function parseSliderTheme(raw: string | null): SliderTheme {
