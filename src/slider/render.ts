...
interface Palette {
  ...
  /** Vivid blob colours. */
  blobs: string[]
  /** Colour of the specks and streaks. */
  speck: string
  /** The slot's fill — a plain darkened silhouette, no outline. */
  slotFill: string
  /** The piece's own edge, so a cut-out reads as something you can pick up. */
  pieceStroke: string
}

const PALETTES: Record<SliderTheme, Palette> = {
  light: {
    from: '#dce6f7',
    to: '#a9bcdd',
 ...
 */
function textureMarkup(random: () => number, palette: Palette): string {
  const parts: string[] = []
  for (let i = 0; i < 7; i += 1) {
    const colour = palette.blobs[Math.floor(random() * palette.blobs.length)] ?? palette.blobs[0]
    parts.push(
      `<circle cx="${r1(random() * CANVAS_WIDTH)}" cy="${r1(random() * CANVAS_HEIGHT)}"` +
  ...
  for (let i = 0; i < 14; i += 1) {
    ...
    parts.push(
      `<line x1="${r1(x)}" y1="${r1(y)}" x2="${r1(x + length)}" y2="${r1(y)}"` +
...
function gradientDefs(palette: Palette): string {
  return (
    `<defs><linearGradient id="dsh-slider-bg" x1="0" y1="0" x2="1" y2="1">` +
 ...
 */
export function renderPuzzle(seed: number, slot: Slot, theme: SliderTheme = 'light'): RenderedPuzzle {
