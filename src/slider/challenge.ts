 ...
 */
export function pickAnswer(random: () => number = Math.random): Slot {
 ...
 */
export function parseDrop(value: unknown): Point | undefined {
  if (typeof value !== 'string' || value.length === 0 || value.length > 24) return undefined
  ...
  if (!numeric.test(rawX) || !numeric.test(rawY)) return undefined
  const x = Number(rawX)
  const y = Number(rawY)
  ...
  if (x > CANVAS_WIDTH - PIECE_SIZE) return undefined
  if (y > CANVAS_HEIGHT - PIECE_SIZE) return undefined
  return { x, y }
}
...
export const ALL_TABS: readonly TabSide[] = ['right', 'left', 'top', 'bottom', 'none']

/** Silhouettes a decoy may take — anything but the real slot's own. */
function decoyTabs(real: TabSide): readonly TabSide[] {
  return ALL_TABS.filter((tab) => tab !== real)
}

/** Bounding-box size of an outline: the body, plus the tab where it has one. */
export function outlineSize(tab: TabSide): { w: number; h: number } {
  if (tab === 'left' || tab === 'right') return { w: PIECE_WIDTH, h: PIECE_SIZE }
  if (tab === 'top' || tab === 'bottom') return { w: PIECE_SIZE, h: PIECE_WIDTH }
 ...
 */
export function bodyAnchor(slot: Slot): Point {
  if (slot.tab === 'left') return { x: slot.x + TAB_RADIUS, y: slot.y }
  if (slot.tab === 'top') return { x: slot.x, y: slot.y + TAB_RADIUS }
 ...
 */
export function piecePath(x: number, y: number, tab: TabSide): string {
  ...
  seg.push(`M ${x + c} ${y}`)
  if (tab === 'top') seg.push(`H ${mx - r}`, `A ${r} ${r} 0 0 1 ${mx + r} ${y}`)
  seg.push(`H ${x + size - c}`)
  seg.push(`Q ${x + size} ${y} ${x + size} ${y + c}`)
  if (tab === 'right') seg.push(`V ${my - r}`, `A ${r} ${r} 0 0 1 ${x + size} ${my + r}`)
  seg.push(`V ${y + size - c}`)
  seg.push(`Q ${x + size} ${y + size} ${x + size - c} ${y + size}`)
  if (tab === 'bottom') seg.push(`H ${mx + r}`, `A ${r} ${r} 0 0 1 ${mx - r} ${y + size}`)
  seg.push(`H ${x + c}`)
  seg.push(`Q ${x} ${y + size} ${x} ${y + size - c}`)
  if (tab === 'left') seg.push(`V ${my + r}`, `A ${r} ${r} 0 0 1 ${x} ${my - r}`)
  seg.push(`V ${y + c}`)
 ...
 */
export function outlineBox(x: number, y: number, tab: TabSide): Box {
  const r = TAB_RADIUS
  let bx = x
  let by = y
  let w = PIECE_SIZE
  let h = PIECE_SIZE
  if (tab === 'left') { bx -= r; w += r }
  if (tab === 'right') { w += r }
  if (tab === 'top') { by -= r; h += r }
  if (tab === 'bottom') { h += r }
 ...
 */
export function decoyBox(decoy: Decoy): Box {
 ...
 * @returns its box in canvas units.
 */
export function slotBox(slot: Slot): Box {
...
function clearOf(a: Box, b: Box, gap: number): boolean {
  return (
    a.x + a.w + gap <= b.x ||
    b.x + b.w + gap <= a.x ||
    a.y + a.h + gap <= b.y ||
 ...
 * @returns the decoy.
 */
export function pickDecoy(slot: Slot, random: () => number = Math.random): Decoy {
  ...
  const start: Box = { x: START_X, y: START_Y, w: size.w, h: size.h }
  ...
  for (let tries = 0; tries < 64; tries += 1) {
    ...
    const decoy: Decoy = { x: pickInt(xMin, xMax), y: pickInt(yMin, yMax), tab }
    const box = decoyBox(decoy)
    if (clearOf(box, real, DECOY_GAP) && clearOf(box, start, DECOY_GAP)) return decoy
  }
