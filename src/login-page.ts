 ...
 * - Dark mode via prefers-color-scheme (standalone page, no shell tokens).
 ...
 * - Renders the optional slider puzzle: the piece is a plain `<img>` dragged
 *   with pointer events over the background, and the drop offset is reported
 *   with the login attempt. Geometry comes from the server so nothing here
 *   hardcodes a canvas size.
 */

import { CANVAS_HEIGHT, CANVAS_WIDTH } from './slider/challenge.ts'
...
<head>
  ...
  <style>
    :root {
      /* Follow the OS/browser light-dark preference; login is a standalone
         document with no access to the DSH shell theme tokens. */
      ...
      --dsh-error: #d4380d;
      --dsh-card-shadow: 0 8px 30px rgba(0, 0, 0, 0.06);
      /* HARNESS badge plate + knocked-out label (always opposite of plate). */
      --dsh-wordmark-plate: #111111;
    ...
    @media (prefers-color-scheme: dark) {
      :root {
        ...
        --dsh-input-bg: #1a1a1a;
        --dsh-accent: #6b85ff;
        --dsh-accent-hover: #8aa0ff;
        --dsh-error: #ff7875;
        --dsh-card-shadow: 0 8px 30px rgba(0, 0, 0, 0.45);
        --dsh-wordmark-plate: #f0f0f0;
    ...
    * {
    ...
    /* Slider puzzle (opt-in, off by default). The stage is a positioning
       context; both images scale with it, so the widget works at any width. */
    .slider-stage {
      position: relative;
      width: 100%;
      line-height: 0;
    ...
    /* Percentages of the stage, so no script needs to know the display scale
       in order to place it — only to convert a drag distance. */
    #slider-piece {
      position: absolute;
      cursor: grab;
      /* Without this a touch drag scrolls the page instead of moving the piece. */
      touch-action: none;
    ...
    /* Overrides the full-width accent button rule above. */
...
<body>
  <form class="card" id="auth-form">
    ...
    <h1 id="form-title">
      <svg class="brand-wordmark" width="171" height="17" viewBox="26 4.5 156 15.5" fill="none" aria-hidden="true">
        ...
        <rect x="129.348" y="5.5" width="52" height="14" rx="2" fill="var(--dsh-wordmark-plate)"></rect>
    ...
    <!-- Shown only when /api/auth/challenge reports the switch on *and* the
         page is in login mode: registration is never gated. -->
    <div class="field" id="challenge-field" style="display:none">
      <label for="slider-piece">拼图验证</label>
      <div class="slider-stage" id="slider-stage">
        <img id="slider-bg" alt="拼图背景，图中有一个和拼图块形状相同的缺口，另一个是干扰项" />
        <img id="slider-piece" alt="拖动这块拼图到缺口处" />
      </div>
      <div class="slider-actions">
        <p class="challenge-note" id="challenge-note">拖动拼图块，对准图中和它形状相同的缺口</p>
        <button type="button" class="captcha-refresh" id="captcha-refresh">换一张</button>
      </div>
    </div>
  ...
  <script>
    (function () {
      ...
      var challengeNote = document.getElementById('challenge-note')
      var sliderStage = document.getElementById('slider-stage')
      var sliderBg = document.getElementById('slider-bg')
      var sliderPiece = document.getElementById('slider-piece')
      var captchaRefresh = document.getElementById('captcha-refresh')
      var mode = 'login'
      // Whether the server wants a puzzle for this login. Stays false until an
      // answer says otherwise: a failed read must not block a login on a
      // deployment that never turned the switch on.
      var sliderEnabled = false
      var challengeId = ''
      // Puzzle geometry, straight from the server (never hardcoded here).
      var canvasWidth = ${CANVAS_WIDTH}
      var canvasHeight = ${CANVAS_HEIGHT}
      var pieceWidth = 0
      var pieceHeight = 0
      // Where the piece sits, in canvas units. It starts at the corner the
      // server names — deliberately not the slot's row, so lining it up is
      // real work on both axes rather than a straight slide.
      var pieceX = 0
      var pieceY = 0
      var startX = 0
      var startY = 0
      ...
      function sliderVisible() {
        return sliderEnabled && mode === 'login'
      }

      function updateSliderVisibility() {
        challengeField.style.display = sliderVisible() ? 'block' : 'none'
      }

      /** Places the piece by percentage, so it tracks the stage at any width. */
      function placePiece() {
        sliderPiece.style.left = (pieceX / canvasWidth * 100) + '%'
        sliderPiece.style.top = (pieceY / canvasHeight * 100) + '%'
        sliderPiece.style.width = (pieceWidth / canvasWidth * 100) + '%'
        sliderPiece.style.height = (pieceHeight / canvasHeight * 100) + '%'
      }

      /**
       * Displayed pixels per canvas unit — the drag delta needs this.
       *
       * One factor for both axes: the stage is the background image's own
       * aspect ratio, so a pixel is the same size whichever way it is measured.
       */
      function stageScale() {
        var rect = sliderStage.getBoundingClientRect()
        return rect.width > 0 ? rect.width / canvasWidth : 1
      }

      function clampX(value) {
        return Math.max(0, Math.min(canvasWidth - pieceWidth, value))
      }

      function clampY(value) {
        return Math.max(0, Math.min(canvasHeight - pieceHeight, value))
      }
      ...
      function onPointerDown(event) {
        ...
        dragStartX = pieceX
        dragStartY = pieceY
        // Keeps move/up events coming to the piece even when the pointer
        // leaves it, which is what makes a fast drag feel right.
        if (sliderPiece.setPointerCapture) sliderPiece.setPointerCapture(event.pointerId)
        sliderPiece.className = 'dragging'
      ...
      function onPointerMove(event) {
        ...
        // Free movement on both axes: the slot is not on the piece's row, so
        // the user has to line it up in two dimensions.
        pieceX = clampX(dragStartX + (event.clientX - dragStartPointerX) / scale)
        pieceY = clampY(dragStartY + (event.clientY - dragStartPointerY) / scale)
        placePiece()
        event.preventDefault()
      }
      ...
      function onPointerUp(event) {
        if (!dragging) return
        dragging = false
        sliderPiece.className = ''
        if (event.pointerId !== undefined && sliderPiece.releasePointerCapture) {
          try { sliderPiece.releasePointerCapture(event.pointerId) } catch (error) { /* already released */ }
        }
      }

      sliderPiece.addEventListener('pointerdown', onPointerDown)
      sliderPiece.addEventListener('pointermove', onPointerMove)
      sliderPiece.addEventListener('pointerup', onPointerUp)
      sliderPiece.addEventListener('pointercancel', onPointerUp)

      /**
       * Fetches a fresh puzzle (also used by "换一张"). Always resets the piece
       * to its start: the previous drop belonged to the previous picture, and
       * the server consumes a puzzle on every attempt either way.
       */
      function loadChallenge() {
        ...
        return fetch('/api/auth/challenge' + (dark ? '?theme=dark' : ''))
          .then(function (res) { return res.ok ? res.json() : null })
          .then(function (data) {
            if (!data) return
            sliderEnabled = data.enabled === true
            if (sliderEnabled) {
              challengeId = data.id
              sliderBg.src = data.background
              sliderPiece.src = data.piece
          ...
          .catch(function () {
            // Leave the field as it is: if the server does require a puzzle the
            // login attempt below is rejected with 'challengeExpired', which
            // fetches a new one and surfaces the failure there.
          })
      }
      ...
      fetch('/api/auth/status')
        ...
        .then(function (data) {
          // An already authenticated visitor normally belongs back in the app
          // — except before any admin account exists. A loopback caller is
          // implicitly trusted (no credentials, no session), so this page is
          // the *only* browser route to create the first admin: the index
          // never redirects a caller it trusts. Bouncing that caller would
          // close the last door, so keep them here and render registration.
          ...
          // The two reads race; each one re-applies visibility so whichever
          // lands last still shows the field for the mode we ended up in.
          updateSliderVisibility()
        })
        .catch(function () {
      ...
      captchaRefresh.addEventListener('click', function () { void loadChallenge() })
      void loadChallenge()
      ...
      form.addEventListener('submit', function (event) {
        ...
        if (sliderVisible() && !hasDragged) {
          setError('请先拖动拼图块，把它对准缺口')
          return
        }
        ...
        if (sliderVisible()) {
          ...
          payload.challengeAnswer = Math.round(pieceX) + ',' + Math.round(pieceY)
        }
        fetch(endpoint, {
          method: 'POST',
          ...
          .then(function (data) {
            ...
            // Every attempt consumes the challenge server-side, whatever went
            // wrong — the verifier runs *before* the password check precisely
            // so that a wrong password cannot be probed without paying for a
            // challenge. So whatever is on screen is spent, and the retry needs
            // a fresh one; fetching it now means it is solved and waiting by
            // the time the password has been retyped.
            if (data && data.error) void loadChallenge()
            // The puzzle was the thing that failed: make the user retype the
            // password too, so a half-filled form cannot be resubmitted against
            // a fresh puzzle by muscle memory alone.
            if (data && data.challengeExpired) {
              password.value = ''
              password.focus()
            }
          })
