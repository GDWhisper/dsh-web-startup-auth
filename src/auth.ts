...
// ── native browser-auth bridge (dsh 0.1.2) ──────────────────────────────────
//
// dsh 0.1.2's `browser-auth.ts` signs its own `dsh-auth-<sha256(authority)>`
// cookie with a 32-byte secret persisted in the credentials service under
// `credentialKey('client-connection', 'browser-session')`. Every `/api`
// request and the SPA index must carry that cookie (no loopback exemption),
// so a caller our session layer already trusts would bounce off the upstream
// fence without help. The bridge mints the exact same cookie format for such
// callers. This is a deliberate coupling to the upstream cookie format;
// `docs/upgrade-dsh-0.1.2-playbook.md` says to re-diff `browser-auth.ts`
// after every dsh upgrade.
//
// The secret is read lazily (the connection plugin creates it during its own
// apply, which may race ours) and cached after the first successful read.
// We never create it ourselves: the key belongs to upstream.

/** Credentials service shape this plugin reads (structural, no type merge). */
...
let lookupCredentials: (() => BrowserSessionCredentials | undefined) | undefined
 ...
 */
const browserSecretCache = new WeakMap<object, Buffer>()
...
function readBrowserCookie(headers: IncomingMessage['headers'], authority: string): string | undefined {
 ...
 */
async function browserAuthSecret(): Promise<Buffer | undefined> {
  const credentials = lookupCredentials?.()
  if (credentials === undefined) return undefined
  const cached = browserSecretCache.get(credentials)
  if (cached !== undefined) return cached
  try {
    const record = await credentials.readRecord(BROWSER_AUTH_RECORD_KEY) as
    ...
    if (secret.length !== BROWSER_SECRET_BYTES) return undefined
...
function browserCookieAttributes(expiresAt: number): string {
 ...
 */
async function mintBrowserCookie(req: IncomingMessage): Promise<string | undefined> {
 ...
 */
function clearBrowserCookie(req: IncomingMessage): string | undefined {
  const authority = requestAuthority(req.headers)
  if (authority === undefined) return undefined
...
/**
 * Minimal bounce document for the native-cookie mint on page navigations.
 *
 * Setting the cookie on a 200 and letting the document navigate itself is
 * deliberate. Browsers do not attach a cookie set by a redirect response to
 * that redirect's target request (WebKit #219650, Firefox #1465402) and a
 * `SameSite=Strict` cookie is withheld on a cross-site-initiated navigation,
 * so a 3xx mint is re-issued on every hop of the chain and the browser ends
 * up at `ERR_TOO_MANY_REDIRECTS`. A 200 stores the cookie, and the
 * meta-refresh navigation that follows is issued by this document, so the
 * cookie is sent on the very next request even when it is `Strict`.
 */
function mintBounceDocument(url: string): string {
  const escaped = url
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
export function apply(ctx: Context, _config: Config): void {
  ...
  lookupCredentials = () => (ctx as unknown as { get(key: string): unknown }).get('credentials') as
  ...
  const authEndpoints: WebRoute[] = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        if (nativeClear !== undefined) cookies.push(nativeClear)
        res.writeHead(200, { 'content-type': 'application/json', 'set-cookie': cookies })
        res.end(JSON.stringify({ ok: true }))
      },
    },
...
;(async function () {
  ...
  // page navigations receive a 200 bounce document carrying the Set-Cookie
  // that navigates itself back to the same URL (see `mintBounceDocument` —
  // a 3xx mint is replayed on every hop by Safari/Firefox and ends in
  // ERR_TOO_MANY_REDIRECTS), so the very next request clears the native
  // fence. Non-navigation GET/HEAD callers keep the 3xx hop, which
  // fetch-style clients follow transparently. Non-navigations (RPC) are
  // forwarded untouched: the browser already holds the cookie from the page
  // hop, and a 303 would turn an RPC POST into a GET. While the upstream
  // signing secret has not appeared yet (the connection plugin creates it in
  // its own apply), minting is skipped and the next request retries.
  //
      ...
      if (authority !== undefined && readBrowserCookie(req.headers, authority) === undefined &&
        ...
        if (minted !== undefined) {
          ...
          if (isPageNavigation(req)) {
            ...
            res.end(req.method === 'HEAD' ? undefined : mintBounceDocument(url))
            return
          }
          res.writeHead(303, {
            location: url,
  ...
  if (tables.fallback !== undefined) tables.fallback = wrapFallback(tables.fallback)
