...
- **Remote-scenario fixes** (two LAN/HTTP pitfalls):
  ...
  - Native browser-session cookie co-signing — since dsh 0.1.2 the `/api` and index pages enforce a native BrowserAuth double gate (`dsh-auth-<authority>` cookie, no loopback exemption); this plugin co-signs the native cookie on login/register/password-change/username-change, and the route wrapper injects a freshly minted token into the **same request** for every authorized caller that lacks one (persisted via Set-Cookie), covering every registered route and upgrade handshake, including routes registered by third-party plugins that activated earlier.
