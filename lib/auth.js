...
import { isValidSessionMaxAgeDays, SESSION_MAX_AGE_CHOICES } from "./session-limits.js";
import { WEB_STARTUP_SERVICE } from "./startup.js";
import { LOGIN_PAGE_HTML } from "./login-page.js";
import { issueSlider, parseSliderTheme, verifySlider } from "./slider/index.js";
import { hasCredentials, registerCredentials, validateCredentials, signSession, verifySession, hardenCredentialFilePermissions, changePassword, changeUsername, getUsername, getRequireLoopbackLogin, setRequireLoopbackLogin, getSlideVerification, setSlideVerification, getSessionMaxAgeDays, setSessionMaxAgeDays, normalizeUsername, MIN_PASSWORD_LENGTH, } from "./credential-store.js";
...
function globalBackoffMs(failures) {
    if (failures < GLOBAL_FREE_FAILURES)
        return 0;
 ...
 */
function isGlobalLockoutExempt(req) {
 ...
 */
function isGloballyLockedOut(req) {
...
function rateLimitedResponse(req, res) {
    ...
    jsonResponse(res, 429, { error: '尝试次数过多，请稍后再试' }, retryAfter === undefined ? {} : { 'retry-after': String(retryAfter) });
}
/**
 ...
 */
function mintBounceDocument(url) {
    const escaped = url
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
function isPageNavigation(req) {
    ...
    if (typeof mode === 'string' && mode !== 'navigate' && mode !== 'nested-navigate')
        return false;
    if (mode === undefined) {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (isLockedOut(req) || globalBackoff) {
                        ctx.logger.warn('web-auth: login rate-limited (%s, from %s)', globalBackoff ? 'global backoff' : 'client lockout', clientIp(req) ?? 'unknown');
                    ...
                    if (getSlideVerification()) {
                        const verdict = verifySlider(clientIp(req), typeof body.challengeId === 'string' ? body.challengeId : '', typeof body.challengeAnswer === 'string' ? body.challengeAnswer : '');
                        if (!verdict.ok) {
                            recordLoginFailure(req);
                            ctx.logger.warn('web-auth: slider rejected (%s, from %s)', verdict.reason, clientIp(req) ?? 'unknown');
...
;(async function () {
  try {
     ...
     */
    const isPublicRoute = (path) => path === '/login' || path === '/oauth/callback' || path.startsWith('/api/auth/');
