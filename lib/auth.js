...
function requestAuthority(host) {
    if (host === undefined)
        return undefined;
    try {
...
function nativeCookieName(authority) {
...
async function browserSessionSecret(ctx) {
    ...
    try {
        ...
        ctx.logger.warn('web-auth: native browser-session secret unavailable: %s', error instanceof Error ? error.message : String(error));
        return undefined;
    }
}
...
function hasNativeCookie(req, authority) {
    if (authority === undefined || req.headers.cookie === undefined)
        return false;
 ...
 */
function nativeCookieValue(req, secret) {
...
function nativeCookieHeaderFromToken(token) {
...
function nativeCookieHeader(req, secret) {
...
function healNativeCookie(req, res, token) {
    ...
    const values = existingSet === undefined ? [] : Array.isArray(existingSet) ? existingSet : [String(existingSet)];
...
function nativeCookieClearHeader(req) {
    const authority = requestAuthority(req.headers.host);
    if (authority === undefined)
        return undefined;
 ...
 */
async function issueAuthCookies(ctx, req, username) {
    ...
    return [sessionCookieSet(session), native].filter((c) => c !== undefined);
}
    ...
    const wrapHandler = (route) => {
        ...
        route.handler = async (req, res) => {
            ...
            if (authority !== undefined && !hasNativeCookie(req, authority)) {
                ...
                if (token !== undefined)
                    healNativeCookie(req, res, token);
            }
